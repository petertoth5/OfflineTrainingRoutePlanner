...
**Last Updated**: 2026-06-01 — Added **colored waypoint markers with improved visibility**: `MarkerIconGenerator` class generates high-contrast circle icons at runtime (red "S" start, green "E" end, yellow "1/2/3" mids) with white 3px borders and white text labels. `MapManager.addMarker()` extended to accept `colorResId` parameter. Redesigned launcher icon combining running figure (left) + route path visualization (right) on primary blue background. All marker colors updated in palette.
